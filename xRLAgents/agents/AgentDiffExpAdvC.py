...
class AgentDiffExpAdvC(): 
    def __init__(self, envs, Config, Model):
        ...
        self.past_buffer_size       = config.past_buffer_size
        self.past_buffer_prob       = config.past_buffer_prob
        ...
        self.past_buffer = torch.zeros((self.past_buffer_size, ) + self.state_shape, dtype=self.dtype)
        ...
        print("past_buffer_size     ", self.past_buffer_size)
        print("past_buffer_prob     ", self.past_buffer_prob)
    ...
    def train(self): 
        ...
        for batch_idx in range(batch_count):        
            ...
            states_seq, _ = self.trajectory_buffer.sample_states_seq(self.ss_batch_size, self.time_distances, self.device)
            states_past   = self._sample_past_states(self.ss_batch_size)

            loss_ssl, info_ssl = self.im_ssl_loss(self.model, states_seq, states_past)
    ...
    def _sample_past_states(self, batch_size):
        indices = torch.randint(0, self.past_buffer_size, (batch_size, ))
