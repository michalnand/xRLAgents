...
class AgentPPOCInDiff():
    def __init__(self, envs, Config, Model):
        ...
        self.z_context  = torch.zeros((self.envs_count, self.context_size, self.n_features), dtype=torch.float32, device=self.device)
    ...
    def step(self, states, training_enabled):     
        ...
        rewards_int, _, self.z_context = self._internal_motivation(states_t, self.alpha_inf, self.alpha_inf, self.z_context)
    ...
    def train(self): 
        ...
        for batch_idx in range(batch_count):    
            ...
            states_seq            = self.trajectory_buffer.sample_trajectory_states(self.context_size, self.ss_batch_size, self.device)
            _, loss_diffusion, _  = self._internal_motivation(states_seq, self.alpha_min, self.alpha_max, None)
    ...
    def _internal_motivation(self, states, alpha_min, alpha_max, z_context):
        ...
        noise_pred = self.model.forward_im_diffusion(z_in_noised)
        ...
        return novelty.detach(), loss, z_context_new
