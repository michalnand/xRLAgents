...
class FrontierStatesBuffer:

    def __init__(self, size):

        self.episode_score = numpy.zeros((size, ))
        self.env_states = []
        for n in range(size):
            self.env_states.append(None)


    def add(self, episode_score, env):

        candidate_idx = numpy.argmin(self.episode_score)

        diff = numpy.abs(episode_score - self.episode_score)
        is_new_score = (diff > 0.01).all()
        
        if is_new_score or (self.env_states[candidate_idx] is None):
            self.episode_score[candidate_idx]   = episode_score
            self.env_states[candidate_idx]      = env.save_state()

            print("saving new state ", candidate_idx, episode_score)
...
class AgentDiffExpFrontier(): 
    def __init__(self, envs, Config, Model):
        ...
        self.frontier_buffer_size   = config.frontier_buffer_size
        self.frontier_ratio         = config.frontier_ratio
        ...
        self.frontier_states_buffer = FrontierStatesBuffer(self.frontier_buffer_size)
        ...
        print("frontier_buffer_size ", self.frontier_buffer_size)
        print("frontier_ratio       ", self.frontier_ratio)
        ...
        state, _, _, _ = self.envs[10].step(0)
    ...
    def step(self, states, training_enabled):
        ...
        # if good episode score, store into buffer as promissing state
        self.episode_score.step(rewards_ext, dones)

        reward_idx = numpy.where(rewards_ext)[0]
        for i in reward_idx:
            self.frontier_states_buffer.add(self.episode_score.episode_score[i], self.envs[i])
        ...
        self.done_idx = numpy.where(dones)[0]
        for i in self.done_idx:
