...
class AgentDiffExpE(): 
    ...
    def step(self, states, training_enabled):     
        ...
        # internal motivation
        rewards_int_a, _ = self._diffusion_im_func(states_t, self.alpha_inf, self.alpha_inf, self.denoising_steps)
        rewards_int_a = rewards_int_a.float().detach().cpu()

        rewards_int_b, _ = self._discriminator_im_func(states_t, None)
        ...
        self.log_rewards_int.add("mean", rewards_int.numpy().mean())
        self.log_rewards_int.add("std",  rewards_int.numpy().std())
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                states_old  = self._sample_buffer(self.ss_batch_size, self.device)
    ...
    def _discriminator_im_func(self, states, labels):
        # single frame input for internal motivation
        # dont use frame stacking, just copy current frame
        if self.im_single_frame:
            states_tmp = torch.zeros_like(states)
        ...
        # obtain taget features
        _, z  = self.model.forward_features(states_tmp)
        z     = z.detach()

        # binary classification, sigmoid output
        y =  self.model.forward_im_discriminator(z)
        y = y.squeeze(1)
