...
class AgentDiffExpD(): 
    ...
    def step(self, states, training_enabled):     
        ...
        # dummy internal motivation is computed in training loop
        ...
        if training_enabled:   
            ...
            if self.rnn_policy:
                self.trajectory_buffer.add(states=states_t, logits=logits_t, values_ext=values_ext_t, values_int=values_int_t, actions=actions, rewards_ext=rewards_ext_scaled, rewards_int=rewards_int, dones=dones, steps=self.episode_steps, hidden_state=self.hidden_state_t)
            else:
                self.trajectory_buffer.add(states=states_t, logits=logits_t, values_ext=values_ext_t, values_int=values_int_t, actions=actions, rewards_ext=rewards_ext_scaled, rewards_int=rewards_int, dones=dones, steps=self.episode_steps)
    ...
    def internal_motivation(self):
        ...
        # iterate truth all environments
        for n in range(self.n_envs):
            # obtain current states for current environment
            states_curr = self.trajectory_buffer.buffer["states"][:, n]
            states_curr = states_curr.to(self.device)

            # diffusion internal motivation

            rewards_int_a, _ = self._diffusion_im_func(states_curr, self.alpha_inf, self.alpha_inf, self.denoising_steps)
            rewards_int_a = rewards_int_a.float().detach().cpu()


            # contextual internal motivation
            ...
            # log results
            self.log_rewards_int.add("mean_a", rewards_int_a.numpy().mean())
            self.log_rewards_int.add("std_a",  rewards_int_a.numpy().std())
            self.log_rewards_int.add("mean_b", rewards_int_b.numpy().mean())
            self.log_rewards_int.add("std_b",  rewards_int_b.numpy().std())
            self.log_rewards_int.add("mean", rewards_int_scaled.numpy().mean())
            self.log_rewards_int.add("std",  rewards_int_scaled.numpy().std())
    ...
    def _contextual_features(self, forward_func, samples, batch_size):
        ...
        z_result = torch.cat(z_result, dim=0)

        return z_result
    

    def _contextual_im(self, states_curr, z_context_norm):
        # obtain current features, note : states_curr are already normalised from buffer
        z_curr      = self._contextual_features(self.model.forward_features, states_curr, self.ss_batch_size, False)
        z_curr_norm = torch.nn.functional.normalize(z_curr)


        # result shape (steps, buffer_size)
        sim = z_curr_norm@z_context_norm.T
        sim = torch.clip(sim, 0.0, 1.0)

        # maximum similarity of current states with contextual states
        # internal motivation is inverse to maximum similarity value
        rewards_int = 1.0 - torch.max(sim, dim=1)[0]

        return rewards_int


    # state denoising ability novely detection
    def _diffusion_im_func(self, states, alpha_min, alpha_max, denoising_steps):
