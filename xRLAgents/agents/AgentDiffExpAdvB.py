...
class AgentDiffExpAdvB(): 
    def __init__(self, envs, Config, Model):
        ...
        if self.state_normalise:
            ...
            for e in range(self.n_envs):
                ...
                state_t = torch.from_numpy(state[0]).to(self.dtype)
                self.state_mean+= state_t.to(self.device)

                self.states_buffer[e] = state_t
    ...
    def step(self, states, training_enabled):     
        ...
        rewards_int_a, rewards_int_b, _  = self._internal_motivation(states_t, self.alpha_inf, self.alpha_inf, self.denoising_steps)
        ...
        self.log_rewards_int.add("std_a",  rewards_int_b.std())
    ...
    def train(self): 
        ...
        for batch_idx in range(batch_count):        
            ...
            _, _, loss_diffusion  = self._internal_motivation(states, self.alpha_min, self.alpha_max, self.denoising_steps)
            ...
            # discriminator loss
            loss_disc, acc_disc = self._discriminator_loss(states, self.states_buffer)
    
            loss = loss_diffusion + loss_ssl + loss_disc
    ...
    def _states_buffer_compute_features(self, states_buffer, batch_size):
        buffer_size = states_buffer.shape[0]
        result_features = None

        for start in range(0, buffer_size, batch_size):
            end = start + batch_size
            batch = states_buffer[start:end].to(self.device)

            z = self.model.forward_im_features(batch)
            z = z.detach().cpu()

            # allocate new buffer if needed
            if result_features is None:
                result_features = torch.zeros((buffer_size, z.shape[-1]), dtype=z.dtype)

            result_features[start:end] = z
