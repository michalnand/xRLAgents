...
def _env_process_main(id, n_envs, env_names, Wrapper, render_mode, child_conn):
    envs = []
    for n in range(n_envs):
        env_name = env_names[n%len(env_names)]
    ...
    while True:
        ...
        if command == "reset":
            ...
            for n in range(n_envs): 
                action = actions[n]%envs[n].action_space.n
                state_, reward_, done_, truncated_, info_ = envs[n].step(action)
...
'''
class MultiEnvsListParallel:
    def __init__(self, env_names, n_envs, render_mode = None, Wrapper = None, n_threads = 4):
        n_actions = 0

        for n in range(len(env_names)):
            if isinstance(env_names[n], str):
                env = gym.make(env_names[n], render_mode=render_mode)
            else:
                env = env_names[n]()
            ...
            if  env.action_space.n > n_actions:
        ...
        for i in range(self.n_threads):
            ...
            worker = multiprocessing.Process(target=_env_process_main, args=(i, self.envs_per_thread, env_names, Wrapper, render_mode, child_conn))
            worker.daemon = True
