...
class AgentDiffExpSurprise: 
    def __init__(self, envs, Config, Model):
        ...
        self.fm_unroll_steps      = config.fm_unroll_steps
        ...
        self.log_loss_surprise = ValuesLogger("loss_causality")
        ...
        print("fm_unroll_steps      ", self.fm_unroll_steps)
    ...
    def get_logs(self):
        ...
        logs = [self.log_rewards_int, self.log_loss_ppo, self.log_loss_diffusion, self.log_loss_surprise, self.log_loss_im_ssl]
    ...
    def train(self): 
        ...
        for e in range(self.training_epochs):
            for batch_idx in range(batch_count):
                ...
                _, loss_surprise = self._im_surprise(states_curr, states_next, actions)
                ...
                # total loss    
                loss = loss_ppo + loss_diffusion + loss_surprise + loss_ssl
                ...
                self.log_loss_surprise.add("loss_surprise", loss_surprise.float().detach().cpu().numpy())
    ...
    def _im_diffusion(self, states, alpha_min, alpha_max, denoising_steps):
    ...
    def _im_surprise(self, states_now, states_next, actions):
        # single frame input for internal motivation
        # dont use frame stacking, just copy current frame
        if self.im_single_frame:
